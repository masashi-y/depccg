...
from pqueue cimport PQueue, pqueue_new
...
cdef class AStarParser(object):
    ...
    cdef object _parse(self, list supertags):
        ...
        for i, stags in enumerate(supertags):
            for parse, in_prob in stags:
                out_prob = out_probs[i, i+1]
                item = agendaitem_new(parse, in_prob, out_prob, i, 1)
                pqueue_enqueue(agenda, item)

        cdef list chart = [ChartCell() for _ in range(s_len * s_len)]
