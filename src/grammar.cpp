...
#define F cat::Slashes::Fwd()
#define B cat::Slashes::Bwd()
...
const std::unordered_set<Cat> ja::possible_root_cats = {
    cat::Parse("NP[case=nc,mod=nm,fin=f]{I1}"),    // 170
    cat::Parse("NP[case=nc,mod=nm,fin=t]{I1}"),    // 2972
    cat::Parse("S[mod=nm,form=attr,fin=t]{I1}"),   // 2
    cat::Parse("S[mod=nm,form=base,fin=f]{I1}"),   // 68
    cat::Parse("S[mod=nm,form=base,fin=t]{I1}"),   // 19312
    cat::Parse("S[mod=nm,form=cont,fin=f]{I1}"),   // 3
    cat::Parse("S[mod=nm,form=cont,fin=t]{I1}"),   // 36
    cat::Parse("S[mod=nm,form=da,fin=f]{I1}"),     // 1
    cat::Parse("S[mod=nm,form=da,fin=t]{I1}"),     // 68
    cat::Parse("S[mod=nm,form=hyp,fin=t]{I1}"),    // 1
    cat::Parse("S[mod=nm,form=imp,fin=f]{I1}"),    // 3
    cat::Parse("S[mod=nm,form=imp,fin=t]{I1}"),    // 15
    cat::Parse("S[mod=nm,form=r,fin=t]{I1}"),      // 2
    cat::Parse("S[mod=nm,form=s,fin=t]{I1}"),      // 1
    cat::Parse("S[mod=nm,form=stem,fin=f]{I1}"),   // 11
    cat::Parse("S[mod=nm,form=stem,fin=t]{I1}")    // 710
};
